...
// The fc_code_emails pass: mailing their code to the FranceConnect beneficiaries who now hold one.
// What every test here is really about is that a code goes out ONCE — the pass runs after every
// FranceConnect pipeline run, over a table that keeps the rows it has already served.

let stack: Stack;

beforeAll(async () => {
  stack = await startStack();
  // Defeat the cooldown: these tests run several passes back to back, and a row touched one
...
describe("fc code emails", () => {
  it("mails the code once, with the template the situation names", async () => {
    const id = await stack.seedConfirmedRow({ sub: "sub-aah", code: "PSP-AAH", situation: "AAH" });

    await stack.enqueueFcCodeEmailsAndWait();
  ...
  it("programs the mail half an hour out, unless the delay is set to 0", async () => {
    await stack.seedConfirmedRow({ sub: "sub-delay", code: "PSP-DELAY", situation: "AAH" });
    ...
    await stack.seedConfirmedRow({ sub: "sub-now", code: "PSP-NOW", situation: "AAH" });
  ...
  it("does not mail the same beneficiary twice across two passes", async () => {
    const id = await stack.seedConfirmedRow({ sub: "sub-once", code: "PSP-ONCE", situation: "boursier" });
  ...
  it("never touches a row from the parcours hors FranceConnect", async () => {
    const id = await stack.seedConfirmedRow({
      sub: "sub-hors-fc",
      code: "PSP-HORS-FC",
      situation: "AAH",
  ...
  it("refuses to guess a template for a self row written before the situation column", async () => {
    const id = await stack.seedConfirmedRow({ sub: "sub-blind", code: "PSP-BLIND", situation: undefined });
  ...
  describe("when Link Mobility refuses", () => {
    ...
    it("stops replaying a row on a terminal error code", async () => {
      const id = await stack.seedConfirmedRow({ sub: "sub-dead", code: "PSP-DEAD", situation: "AAH" });
    ...
    it("replays a row rate-limited by Link Mobility", async () => {
      const id = await stack.seedConfirmedRow({ sub: "sub-slow", code: "PSP-SLOW", situation: "AAH" });
    ...
    it("replays an unreachable gateway until the ceiling, then no longer selects the row", async () => {
      ...
      const id = await stack.seedConfirmedRow({ sub: "sub-down", code: "PSP-DOWN", situation: "AAH" });
