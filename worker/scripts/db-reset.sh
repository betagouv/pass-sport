#!/usr/bin/env bash
#
# Local dev only. Empties the four data tables in the compose `db` service.
...
docker compose -f "$COMPOSE_FILE" exec -T db \
  ...
  'TRUNCATE audit, eligibility_history, eligibility_results, email_verifications RESTART IDENTITY CASCADE;'
