"""Unit tests for partners_lib, the logic shared by the 2026 partner cleaning notebooks.

Moved here out of cnaf/test_clean_cnaf_lib.py when MSA started routing through the same
qf-batch pipeline. What is left in a partner's own test module is what only that partner
does (its column mapping, its file quirks).

Run from data/: source .venv/bin/activate && pytest 2026/partners/test_partners_lib.py
"""
...
import partners_lib as lib


# --- Phase 1: pre-checkpoint ------------------------------------------------------

def test_strip_all_string_columns():
    df = pd.DataFrame({
        'NOMENF': ['  DUPONT  ', ' MARTIN'],
        'PRENOMENF': ['Jean ', '  Marie '],
    })

    result = lib.strip_all_string_columns(df)

    assert result['NOMENF'].tolist() == ['DUPONT', 'MARTIN']
    assert result['PRENOMENF'].tolist() == ['Jean', 'Marie']


def test_strip_all_string_columns_does_not_mutate_its_input():
    df = pd.DataFrame({'NOMENF': ['  DUPONT  ']})

    lib.strip_all_string_columns(df)

    assert df['NOMENF'].tolist() == ['  DUPONT  ']
...
def test_set_organisme_and_situation_maps_the_partner_origin():
    df = pd.DataFrame({'situation_origine': ['ARS', 'AAH', 'AEEH', 'UNKNOWN']})

    result = lib.set_organisme_and_situation(
        df, 'CAF', {'ARS': 'jeune', 'AAH': 'AAH', 'AEEH': 'AEEH'})
...
def test_set_organisme_and_situation_accepts_a_partner_specific_spelling():
    # MSA spells the AEEH route "AEH" and is its own organisme
    df = pd.DataFrame({'situation_origine': ['ARS', 'AAH', 'AEH']})

    result = lib.set_organisme_and_situation(
        df, 'MSA', {'ARS': 'jeune', 'AAH': 'AAH', 'AEH': 'AEEH'})

    assert result['organisme'].tolist() == ['MSA'] * 3
    assert result['situation'].tolist() == ['jeune', 'AAH', 'AEEH']
...
def test_parse_beneficiary_birthdate_accepts_another_partner_format():
    df = pd.DataFrame({'date_naissance': ['20100315', '20091201']})

    result = lib.parse_beneficiary_birthdate(df, '%Y%m%d')
...
def _qf_route_frame(rows):
    """Frame shaped like the mapped partner data, for the qf-batch input steps."""
...
def test_format_qf_identity_fields_keeps_a_nom_usage_the_partner_already_set():
    # MSA reads the usage name off its destinataire columns, before calling this
    df = pd.DataFrame({
        'allocataire-nom': ['VELTRANO'],
        'allocataire-nom_usage': ['ZANDRIC'],
        'allocataire-genre': ['F'],
        'allocataire-date_naissance': ['12/03/1980'],
    })

    result = lib.format_qf_identity_fields(df)

    assert result['allocataire-nom_usage'].tolist() == ['ZANDRIC']
...
# utils/data_utils.py - covered by utils/test_data_utils.py. Only their partner use
# (birth country label -> allocataire-code_pays_naissance) is tested here.
...
def test_add_allocataire_json_column_without_extra_fields_carries_only_the_shared_core():
    """Guards the CNAF export against the extra_fields parameter added for MSA."""
    df = pd.DataFrame([_beneficiary_row(**{'allocataire-date_naissance': '30/04/1985',
                                           'allocataire-pays_naissance': 'FRANCE'})])

    result = lib.add_allocataire_json_column(df)

    assert sorted(json.loads(result['allocataire'].iloc[0])) == [
        'code_organisme', 'courriel', 'matricule', 'nom', 'prenom', 'qualite', 'telephone']
...
def test_add_allocataire_json_column_carries_the_partner_extra_fields():
    ...
    allocataire = json.loads(result['allocataire'].iloc[0])
    assert allocataire['date_naissance'] == '30/04/1985'
    # a null extra field is omitted like any other
    assert 'pays_naissance' not in allocataire
...
def test_add_adresse_allocataire_json_column_without_extra_fields_carries_only_the_shared_core():
    """Guards the CNAF export against the extra_fields parameter added for MSA."""
    df = pd.DataFrame([_address_row(**{'adresse_allocataire-nom_adresse_postale': 'M DUPONT JEAN'})])

    result = lib.add_adresse_allocataire_json_column(df)

    assert sorted(json.loads(result['adresse_allocataire'].iloc[0])) == [
        'code_insee', 'code_postal', 'commune', 'cplt_adresse', 'voie']


def test_add_adresse_allocataire_json_column_carries_the_partner_extra_fields():
    df = pd.DataFrame([_address_row(**{'adresse_allocataire-nom_adresse_postale': 'M DUPONT JEAN'})])

    result = lib.add_adresse_allocataire_json_column(
        df, {'nom_adresse_postale': 'adresse_allocataire-nom_adresse_postale'})

    adresse = json.loads(result['adresse_allocataire'].iloc[0])
    assert adresse['nom_adresse_postale'] == 'M DUPONT JEAN'
    # the shared core is untouched by the extra field
    assert adresse['code_postal'] == '07501'
...
def test_drop_intermediate_columns_accepts_a_partner_extended_list():
    df = pd.DataFrame([{column: 'x' for column in lib.FINAL_COLUMNS_TO_DROP}
                       | {'allocataire-commune_naissance': 'ANGERS', 'nom': 'DUPONT'}])

    result = lib.drop_intermediate_columns(
        df, lib.FINAL_COLUMNS_TO_DROP + ['allocataire-commune_naissance'])
