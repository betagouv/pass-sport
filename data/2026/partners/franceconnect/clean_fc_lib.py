...
# Ce que le passage de rattrapage (worker/src/scripts/qf-batch.ts) attend en entrée :
# `allocataire_fc_sub` comme clé de réinjection, puis ses IDENTITY_COLUMNS au nom près.
# qf-batch vérifie ses colonnes obligatoires sur la première ligne du CSV et refuse de
# démarrer au moindre écart — ce contrat est donc vérifié avant le premier appel d'API.
CONJOINT_RECALL_COLUMNS = [
    'allocataire_fc_sub',
    'allocataire-nom_naissance',
    'allocataire-prenom',
    'allocataire-date_naissance',
...
# Les deux routes dont le rapprochement compare un allocataire : elles seules gagnent quelque
# chose à connaître le conjoint. AAH compare le bénéficiaire lui-même, boursier son INE.
CONJOINT_RECALL_SITUATIONS = ('jeune', 'AEEH')


def _allocataires_du_foyer(valeur) -> list:
    """Le tableau `allocataires` d'une réponse quotient_familial, [] s'il manque ou est illisible."""
    try:
        allocataires = json.loads(valeur) if valeur else []
    except (TypeError, ValueError):
        return []
    if not isinstance(allocataires, list):
        return []
    return [a for a in allocataires if isinstance(a, dict)]


def build_conjoint_recall_rows(df: pd.DataFrame,
                               situations=CONJOINT_RECALL_SITUATIONS) -> pd.DataFrame:
    """Les foyers QF/AEEH à rappeler sur l'API quotient familial, faute de tableau `allocataires`.

    Une ligne AEEH n'a besoin d'aucune réponse quotient_familial pour exister — resolve_situation
    la déduit d'une fenêtre de naissance sans couverture QF — et une ligne `jeune` peut avoir
    perdu la sienne (404 « Erreur inattendue », 429, maintenance). Ces foyers ne présentent
    qu'une persona au rapprochement ; un appel hors ligne peut leur rendre le couple.

    Ne sont retenues QUE les lignes sans tableau exploitable : un foyer à un seul allocataire,
    ou un couple qu'on n'arrive pas à départager, rendrait exactement la même photo.

    Dédoublonné sur `allocataire_fc_sub` : l'appel porte sur le foyer, pas sur l'enfant — trois
    enfants d'un même sub valent un appel, pas trois.

    À appeler sur une photo prise entre resolve_situation et drop_intermediate_columns : c'est
    le seul moment où la `situation` et la charpente `allocataire-*` coexistent.
    """
    if df.empty:
        return pd.DataFrame(columns=CONJOINT_RECALL_COLUMNS)

    sub = df['allocataire_fc_sub'].fillna('').astype(str).str.strip()
    a_rappeler = df[
        df['situation'].isin(situations)
        & (sub != '')
        & df['qf_allocataires'].map(lambda v: not _allocataires_du_foyer(v))
    ]

    rows = pd.DataFrame({
        colonne: (a_rappeler[colonne] if colonne in a_rappeler.columns
                  else pd.Series('', index=a_rappeler.index))
        for colonne in CONJOINT_RECALL_COLUMNS
    })

    return (rows[CONJOINT_RECALL_COLUMNS].fillna('')
            .drop_duplicates(subset='allocataire_fc_sub')
            .reset_index(drop=True))


def count_conjoint_causes(df: pd.DataFrame, situations=CONJOINT_RECALL_SITUATIONS) -> dict:
    """Pourquoi ces lignes QF/AEEH n'ont pas de conjoint — une cause, un compteur.

    Chiffre ce qu'un rattrapage rapporterait AVANT de dépenser du quota : seule la première
    cause est rappelable. À appeler après resolve_allocataire_conjoint, dont les colonnes disent
    si un conjoint a été retenu.
    """
    vide = {
        'conjoints_manquants_sans_allocataires': 0,
        'conjoints_manquants_allocataire_unique': 0,
        'conjoints_manquants_ambigus': 0,
        'foyers_a_rappeler': 0,
    }
    if df.empty:
        return vide

    concernees = df[df['situation'].isin(situations)]
    if concernees.empty:
        return vide

    def colonne(nom: str) -> pd.Series:
        # fillna : build_psp_columns repasse les chaînes vides en NaN (son .mask() final), et
        # cette photo est prise après lui.
        return (concernees[nom].fillna('') if nom in concernees.columns
                else pd.Series('', index=concernees.index))

    entrees = concernees['qf_allocataires'].map(lambda v: len(_allocataires_du_foyer(v)))
    sans_conjoint = (colonne('match-conjoint_nom_naissance') == '') \
        & (colonne('match-conjoint_nom_usage') == '')

    return {
        'conjoints_manquants_sans_allocataires': int((entrees == 0).sum()),
        'conjoints_manquants_allocataire_unique': int((entrees == 1).sum()),
        'conjoints_manquants_ambigus': int(((entrees >= 2) & sans_conjoint).sum()),
        'foyers_a_rappeler': len(build_conjoint_recall_rows(df, situations)),
    }
